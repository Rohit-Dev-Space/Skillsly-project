...
export default function Modal({ isClose, title, type = '', children, className }) {
    ...
    return (
        <div className={`fixed inset-0 bg-black/40 backdrop-blur-xl z-40 p-10 text-white ${className} overflow-y-scroll`}>
            <div className={`${type === 'profile' ? 'bg-black' : 'bg-black/30'} rounded-2xl ${type === 'notification' ? 'h-[500px] w-2/4 pb-20' : 'w-6/7 h-fit'}  mx-auto mt-20 border flex flex-col justify-start items-center`}>
