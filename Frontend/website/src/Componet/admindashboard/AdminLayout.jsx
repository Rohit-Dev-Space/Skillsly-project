    ...
    <div className="flex h-screen bg-black text-white font-sans">
      ...
      <aside className="w-64 border-r border-gray-800 flex flex-col p-6">
        <h1 className="text-2xl font-bold mb-10 text-white">
          ...
          SkillSly
          <span className="text-2xl text-teal-500">ADMIN</span></h1>
        ...
        <nav className="flex-1 space-y-2">
          {navItems.map((item) => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `flex items-center gap-3 p-3 rounded-lg transition ${isActive ? 'bg-teal-900/30 text-teal-400 border-l-4 border-teal-500' : 'text-gray-400 hover:bg-gray-900'
                }`
              }
            >
              {item.icon} {item.name}
