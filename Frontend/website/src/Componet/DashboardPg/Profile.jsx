...
export default function Profile() {
    ...
    return (
                ...
                <div className="flex flex-col md:flex-row space-y-6 md:space-y-0 md:space-x-15 mb-12">
                    ...
                    <div className="flex flex-col items-center md:items-center min-w-[150px]">
                        <div className="relative w-32 h-32 rounded-full border-2 border-gray-700 overflow-hidden bg-gray-700 mb-6">
                            <img src={user?.profileImageUrl} className="object-cover  w-full h-full text-white" />
                        </div>
        ...
        ${user.skillsKnow.length === 1 || 3 ? "col-span-2 mx-auto" : ""}`}                                >
                                    {skill}
                                </span>
                            ))}
                        </div>
                    ...
                    <div className="flex-grow text-white">
                        ...
                        <p className="text-gray-300 leading-relaxed mb-6">
                            {user?.bio || "User didnt care to add bio"}
                        </p>

                        <div className="flex gap-1">
                            <p>Languages known :</p>
                            <p className="pb-5">[{user?.languages.map((lang, index) => <span key={index} className="m-2">{lang}{index === user?.languages.length - 1 ? "" : ", "}</span>) || "No languages specified"}]</p>
                        </div>

                        <p className={`text-sm cursor-${user?.socialLink ? "pointer" : "none"} text-teal-400 mb-8 truncate`}>
                            {user?.socialLink || "No social link provided"}
                        </p>
                ...
                <div className="w-full h-fit overflow-hidden py-10">
                    <Carousel>
                        <CarouselContent className="px-1 sm:px-5 space-x-5 flex items-end">
                            {user?.workImageUrl.map((item, index) => (
                                <CarouselItem
                                    key={index}
